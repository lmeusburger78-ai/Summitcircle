...
export default function ToursPage() {
  ...
  async function loadTourDetails(tourId: string) {
    ...
    const { data: participantData } = await supabase
      ...
      .select(`*, user:profiles!tour_participants_user_id_fkey(id, username)`)
      .eq('tour_id', tourId)
    ...
    if (error) {
      ...
      if (tour) {
        await supabase.from('notifications').insert({
          ...
          actor_id: user.id,
          type: 'tour_request',
          title: 'Neue Tour-Anfrage 🗺️',
          message: `@${profile?.username} möchte an deiner Tour "${tour.title}" teilnehmen`,
          link: '/tours',
        })
      }
  ...
  async function handleParticipantStatus(participantId: string, status: string, userId: string, tourTitle: string) {
    const supabase = createClient()
    await supabase.from('tour_participants').update({ status }).eq('id', participantId)
    await supabase.from('notifications').insert({
      user_id: userId,
      actor_id: user.id,
      type: status === 'confirmed' ? 'tour_confirmed' : 'tour_rejected',
      title: status === 'confirmed' ? 'Tour-Anfrage bestätigt ✅' : 'Tour-Anfrage abgelehnt ❌',
      message: status === 'confirmed'
        ? `Deine Anfrage für "${tourTitle}" wurde bestätigt!`
        : `Deine Anfrage für "${tourTitle}" wurde leider abgelehnt.`,
      link: '/tours',
    })
    ...
    <div style={{ backgroundColor: '#0f1a0f', minHeight: '100vh', fontFamily: 'Arial, sans-serif', color: '#e8f5e8' }}>
      ...
      <div style={{ maxWidth: '700px', margin: '0 auto', padding: '24px' }}>
        ...
        <div style={{ display: 'flex', gap: '12px', marginBottom: '20px', flexWrap: 'wrap' }}>
          <select value={filterBundesland} onChange={(e) => setFilterBundesland(e.target.value)} style={{ padding: '8px 12px', borderRadius: '20px', border: '1px solid #2a3d2a', backgroundColor: '#162016', color: '#e8f5e8', fontSize: '13px', cursor: 'pointer' }}>
            <option value="">🗺️ Alle Bundesländer</option>
            {Object.keys(BUNDESLAENDER).map((bl) => (
              <option key={bl} value={bl}>{bl}</option>
            ))}
          </select>
          <select value={filterAktivitaet} onChange={(e) => setFilterAktivitaet(e.target.value)} style={{ padding: '8px 12px', borderRadius: '20px', border: '1px solid #2a3d2a', backgroundColor: '#162016', color: '#e8f5e8', fontSize: '13px', cursor: 'pointer' }}>
        ...
        {showForm && (
          <div style={{ backgroundColor: '#162016', border: '1px solid #2a3d2a', borderRadius: '12px', padding: '24px', marginBottom: '24px' }}>
            ...
            <select value={selectedBundesland} onChange={(e) => { setSelectedBundesland(e.target.value); setSelectedRegionen([]) }} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1px solid #2a3d2a', backgroundColor: '#0f1a0f', color: 'white', marginBottom: '16px', fontSize: '14px' }}>
            ...
            {selectedBundesland && (
              <>
                <label style={{ color: '#a0b8a0', fontSize: '13px', display: 'block', marginBottom: '8px' }}>Regionen (Mehrfachauswahl möglich)</label>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginBottom: '16px' }}>
                  {BUNDESLAENDER[selectedBundesland].map((region) => (
                    <button key={region} type="button" onClick={() => toggleRegion(region)} style={{ padding: '6px 12px', borderRadius: '20px', border: '1px solid #2a3d2a', backgroundColor: selectedRegionen.includes(region) ? '#4CAF50' : '#0f1a0f', color: selectedRegionen.includes(region) ? '#0f1a0f' : '#a0b8a0', fontSize: '12px', cursor: 'pointer', fontWeight: selectedRegionen.includes(region) ? 'bold' : 'normal' }}>
                      {region}
                    </button>
                  ))}
                </div>
            ...
            <label style={{ color: '#a0b8a0', fontSize: '13px', display: 'block', marginBottom: '6px' }}>🔗 Komoot Link (optional)</label>
            <input type="url" placeholder="https://www.komoot.com/tour/..." value={komootUrl} onChange={(e) => setKomootUrl(e.target.value)} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1px solid #2a3d2a', backgroundColor: '#0f1a0f', color: 'white', marginBottom: '16px', fontSize: '14px' }} />
            <label style={{ color: '#a0b8a0', fontSize: '13px', display: 'block', marginBottom: '6px' }}>📁 GPX Datei hochladen (optional)</label>
            <input type="file" accept=".gpx" onChange={(e) => setGpxFile(e.target.files?.[0] || null)} style={{ width: '100%', padding: '10px', borderRadius: '8px', border: '1px solid #2a3d2a', backgroundColor: '#0f1a0f', color: 'white', marginBottom: '16px', fontSize: '14px' }} />
            {gpxFile && <p style={{ color: '#4CAF50', fontSize: '12px', marginTop: '-12px', marginBottom: '16px' }}>✓ {gpxFile.name} ausgewählt</p>}
          ...
          tours.map((tour) => (
            <div key={tour.id} style={{ backgroundColor: '#162016', border: selectedTour?.id === tour.id ? '1px solid #4CAF50' : '1px solid #2a3d2a', borderRadius: '12px', padding: '20px', marginBottom: '12px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                ...
                <span style={{ backgroundColor: '#1e3a1e', color: '#4CAF50', fontSize: '11px', padding: '3px 10px', borderRadius: '10px' }}>{activityLabels[tour.activity_type]}</span>
              </div>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '8px' }}>
                <a href={`/user/${tour.creator?.username}`} style={{ color: '#6a8a6a', fontSize: '12px', textDecoration: 'none' }}>von @{tour.creator?.username}</a>
                <div style={{ display: 'flex', gap: '8px' }}>
                  <button onClick={() => { if (selectedTour?.id === tour.id) { setSelectedTour(null) } else { setSelectedTour(tour); loadTourDetails(tour.id) } }} style={{ backgroundColor: 'transparent', color: '#a0b8a0', border: '1px solid #2a3d2a', padding: '8px 16px', borderRadius: '20px', fontSize: '13px', cursor: 'pointer' }}>
                  ...
                  {participants.length === 0 ? (
                    ...
                    participants.map((p) => (
                      <div key={p.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px', backgroundColor: '#0f1a0f', borderRadius: '8px', marginBottom: '6px' }}>
                        <a href={`/user/${p.user?.username}`} style={{ fontSize: '13px', color: '#e8f5e8', textDecoration: 'none' }}>@{p.user?.username}</a>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                          ...
                          {tour.creator_id === user?.id && p.status === 'pending' && (
                            <>
                              <button onClick={() => handleParticipantStatus(p.id, 'confirmed', p.user.id, tour.title)} style={{ backgroundColor: '#4CAF50', color: '#0f1a0f', border: 'none', padding: '4px 10px', borderRadius: '12px', fontSize: '11px', cursor: 'pointer' }}>✓</button>
                              <button onClick={() => handleParticipantStatus(p.id, 'rejected', p.user.id, tour.title)} style={{ backgroundColor: 'transparent', color: '#ff6b6b', border: '1px solid #ff6b6b', padding: '4px 10px', borderRadius: '12px', fontSize: '11px', cursor: 'pointer' }}>✕</button>
                            </>
                          )}
                        </div>
                      </div>
                  ...
                  {comments.length === 0 ? (
                    ...
                    comments.map((c) => (
                      <div key={c.id} style={{ padding: '8px 12px', backgroundColor: '#0f1a0f', borderRadius: '8px', marginBottom: '6px' }}>
                        <a href={`/user/${c.author?.username}`} style={{ color: '#4CAF50', fontSize: '12px', fontWeight: 'bold', textDecoration: 'none' }}>@{c.author?.username}</a>
                        <p style={{ fontSize: '13px', margin: '4px 0 0 0' }}>{c.content}</p>
                      </div>
                    ))
                  )}
                  <div style={{ display: 'flex', gap: '8px', marginTop: '8px' }}>
                    <input type="text" placeholder="Kommentar schreiben..." value={newComment} onChange={(e) => setNewComment(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleComment(tour.id)} style={{ flex: 1, padding: '8px 12px', borderRadius: '20px', border: '1px solid #2a3d2a', backgroundColor: '#0f1a0f', color: 'white', fontSize: '13px' }} />
                    <button onClick={() => handleComment(tour.id)} style={{ backgroundColor: '#4CAF50', color: '#0f1a0f', border: 'none', padding: '8px 16px', borderRadius: '20px', fontSize: '13px', fontWeight: 'bold', cursor: 'pointer' }}>Senden</button>
                  </div>
                </div>
              )}
            </div>
